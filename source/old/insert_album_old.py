...
class Album():
    ...
    def print_inserted_data(self):
        if(self.all_success):      
            ...
            print(pd.DataFrame(self.tracks_data),end='\n\n')
    ...
    #         regresa
        ...
        with Session(engine) as session:
            ...
            print('checalo')  
               #try:
            # start transaction
            #bool que cheque si self.album_id existe en rys.album_ratings donde user sea 1




            # q="select album_id from album_ratings where album_id={self.album_id} and user_id=1 limit 1" 
            # album_in_lib = connection.execution(text(q)).scalar()

            # if album_in_lib:
            #     return 220
            
            # q='select album_id from albums where album_id={self.album_id}'
            # album_in_catalog_not_lib = connection.execution(text(q)).scalar()
            # if(album_in_catalog_not_lib):
            #     #insert albvm header
            #     q='insert into '
